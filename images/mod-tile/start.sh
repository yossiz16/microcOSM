    ...
    if [ -f $expiredDirectory/state.txt ]; then
        ...
        if [ -n "$sequenceNumber" ]; then
            ...
            do
                ...
                EXPIRE_FILE=$expiredDirectory/$dir1/$dir2/$state-expire.list
                if [ -f "$EXPIRE_FILE" ]; then
                    chmod 700 $EXPIRE_FILE
                    cat $EXPIRE_FILE >> $expiredDirectory/currentlyExpired.list
                fi
            done
...
function getExternalData () {
    ...
    PGPASSWORD=$POSTGRES_PASSWORD /src/openstreetmap-carto/scripts/get-external-data.py -H $POSTGRES_HOST -d $POSTGRES_DB -p 5432 -U $POSTGRES_USER -c /src/openstreetmap-carto/external-data.yml
}
